...
class NextExamsPage(QMainWindow): # Finestra di visualizzazione degli appelli prenotati
    def __init__(self, matricola: str, home_page):
        ...
        self.show_next_exams() # Legge dal database gli appelli prenotati

        if not self.next_exams:  # Se non ci sono appelli prenotati
            no_exams_label = QLabel("Nessun appello prenotato.")
            ...
            self.table.setRowCount(len(self.next_exams))
            ...
            for row_index, exam in enumerate(self.next_exams):
    ...
    def show_next_exams(self): # Funzione di visualizzazione degli appelli prenotati dallo studente
        try:
            ...
            self.next_exams = cursor.fetchall()  # Risultati della query
        except Error as e:
    ...
    def cancel_reservation(self, row_index):  # Funzione per cancellare una prenotazione
        ...
        if confirmation.clickedButton() == btn_yes:
            try:
                ...
                query = "DELETE FROM PRENOTA WHERE NOMEESAME = %s AND NOMECORSO = %s AND MATRICOLA = %s"
                cursor.execute(query, (exam_name, course_name, self.home_page.matricola))
                conn.commit()
